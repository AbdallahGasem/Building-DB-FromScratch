...
// Description: setting the program flow and the menu 
// Author:  Abdallah Gasem
// Date: 04-12-2024
// Version: 1.0
/* file run command: g++ -o QueryHandlingandProgramFlow QueryHandlingandProgramFlow.cpp; if ($?) { ./QueryHandlingandProgramFlow } */
// ----------------------------------------------------------------------------------------------------------------------------- //
...
vector<vector<string>> queryParser( string query ){
    ...
    if (mainVector.size() < 2 || mainVector[1].empty() || mainVector[0].empty()) {
        cerr << "--- the program aborted by the validator due to invalid query structure! ---\n";
        return {};
    }
...
void queryExecuter(string query){
    ...
    if (targetTable == "doctors")
    {
        ...
        if (selectAtrributes[0] == "all" || selectAtrributes[0] == "doctorid,doctorname,address" || selectAtrributes[0] == "*")
        {
            ...
            searchID(docFile,condAttributeValue.c_str(),true);
            cout << "\n_____________________________________________\n";
        ...
        else if ( selectAtrributes[0] == "doctorname" ) {
            // use secondary index  //
        }
    ...
    else if (targetTable == "appointments")
    {
        ...
        if (selectAtrributes[0] == "all" || selectAtrributes[0] == "appointmentid,appointmentdate,doctorid" || selectAtrributes[0] == "*")
        {
            // use primary index primary index //
            cout << "\n_____________________________________________\n";
            searchID(appFile,condAttribute.c_str(),true);
            cout << "\n_____________________________________________\n";
        ...
        else if ( selectAtrributes[0] == "doctorid" ) {
            // use secondary index  //
        }
...
void launcher(){
    ...
    while (true)
    {
        ...
        cout << "9)  Write Query \n";
        cout << "10) EXIT \n";
        cout << "11) Read Doctors File \n";
        cout << "12) Read Appointments File \n";
        cout << ">> ";
        cin >> option;
        ...
        switch (option)
        {
            ...
            case 6: {
                ...
                // deleteDoctor(docFile, doctorsAvailList, doctorId);
            ...
            case 7 : {
                ...
                // <aka search for the dr using the primary index in the doctor id> 
                // apply the search logic
                cout << "Enter Doctor ID to search for: ";
                cin >> doctorId;
                cout << endl;
                searchID(docFile,doctorId.c_str(),true);
            ...
            case 8: {
                ...
                //<aka search for the appointment using the primary index in the appointment id> 
                cout << "Enter Doctor ID to search for: ";
                cin >> appointmentId;
                cout << endl;
                searchID(appFile,appointmentId.c_str(),false);
            ...
            case 12:
            {
                string s = readAppointments(appFile);
                cout<<s<<endl;
                break;
            }
...
int main() {
    ...
    string query = "SELECT * FROM Doctors WHERE doctorId=1";
    queryExecuter(query); 
